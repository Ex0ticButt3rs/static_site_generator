...
def generate_page(from_path, template_path, dest_path, basepath):
    ...
    with open(template_path, mode='r') as opened_template_object:
        ...
        updated_template_string = template_read_string.replace("{{ Title }}", page_title).replace("{{ Content }}", markdown_to_html_result).replace('href="/', f'href="{basepath}').replace('src="/', f'src="{basepath}')
...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, basepath):
    ...
    for entry in dir_path_content.iterdir():
        if entry.is_dir():
            generate_pages_recursive(entry, template_path, dest_dir_path, basepath)
        if entry.is_file() and entry.suffix == ".md":
            relative = entry.relative_to(Path("content"))
            dest_path = dest_dir_path / relative
            dest_path = dest_path.with_suffix(".html")
            generate_page(str(entry), template_path, str(dest_path), basepath)
